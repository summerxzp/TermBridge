...
| `bootstrap_host` returns `timed_out` | User didn't respond to the password prompt (5 min default). Tell the user a password prompt will appear and re-run `bootstrap_host` when they're at the machine |
| Credential error says "no interactive password prompt available" | Environment problem, not user cancellation. Relay the error's fix options (install zenity/kdialog/yad / set `TERMBRIDGE_ASKPASS` / run from a terminal); do NOT retry in a loop |
...
| `open_session(persistent=true)` → RUNTIME_MISSING | local agentd binary absent (NOT a remote host problem, retrying won't help). Follow the fix guidance in the error message (release archive / npm reinstall / `cargo build`); use `persistent=false` as fallback while agentd is unavailable |
...
- **Credentials never in MCP params**: `bootstrap_host` accepts only `host`. Passwords are prompted via separate `termbridge-auth-helper` process, never exposed to MCP stdio. The prompt is out-of-band: a GUI dialog (zenity/kdialog/yad on Linux, osascript on macOS, CredUI on Windows), a `TERMBRIDGE_ASKPASS` program, or the terminal — depending on the user's environment.
- **Prompt result semantics (ADR-0019)**: `cancelled` = the user dismissed the prompt (ask whether to retry). `timed_out` = the user did not respond within the window (default 5 min) — ask the user and retry when they are present. An `INVALID_ARGUMENT` error containing "no interactive password prompt available" = the user's machine has no input channel at all — relay the error message verbatim (it lists the fix: install zenity/kdialog/yad, set `TERMBRIDGE_ASKPASS`, or run from an interactive terminal); do NOT retry.
...
- **Persistent sessions need agentd (Linux x86_64)**: `open_session(persistent=true)` auto-deploys the TermBridge agentd daemon to the remote host, which requires the agentd binary locally (bundled `resources/agentd/linux-x86_64/termbridge-agentd` next to the exe, or local cache). A `RUNTIME_MISSING` error means the binary is absent — it is NOT a remote host problem and NOT fixable by retrying; the error message itself lists the fix paths (GitHub release archive / npm reinstall / `cargo build --release -p termbridge-agentd`). Standard sessions (`persistent=false`) don't need agentd and remain available as a fallback.
